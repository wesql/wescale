...
The dml job controller is the core component of the non-transactional dml feature, maintaining tables processed by each running or paused job. 

When a new job is submitted, if the table it modifies are not in of the table set maintained by the job controller and the dml job is within its running time period, the job can be scheduled for execution. 

In other words, if two jobs process different tables, they can run in parallel. Currently, there is no limit to the number of jobs that can run in parallel.

The job states include `postpone-launch`, `queued`, `not-in-time-period`, `running`, `paused`, `canceled`, `completed` and `failed`.

You can submit a job and make it in either `postpone-launch` or `queued` status. If a job in the `queued` status is not within the running time period, it transitions to `not-in-time-period` status. 

Jobs meeting the running conditions enter the `running` state. Users can set a running job to `paused` or `canceled`. A `running` job that completes successfully enters the `completed` status. If there is a failure during execution, the job enters the `failed` state.
