...
import (
	...
	"github.com/wesql/wescale/go/vt/vttablet/tabletmanager/vdiff"
...
func buildVDiff2SingleSummary(wr *wrangler.Wrangler, keyspace, workflow, uuid string, output *wrangler.VDiffOutput, verbose bool) (*vdiffSummary, error) {
	...
	if shardStateCounts[vdiff.StoppedState] > 0 {
		...
		// with each one writing to the shared mysql.vdiff_table record for the target shard.
		...
		// shard by setting mysql.vdiff.state = completed.
