...
func newWranglerTestEnv(sourceShards, targetShards []string, query string, positions map[string]string, timeUpdated int64) *testWranglerEnv {
	...
	for _, shard := range targetShards {
		...
		for j, sourceShard := range sourceShards {
			...
			env.tmc.setVRResults(
				primary.tablet,
				fmt.Sprintf("update mysql.vreplication set state='Running', stop_pos='%s', message='synchronizing for wrangler test' where id=%d", testSourceGtid, j+1),
				&sqltypes.Result{},
			)
		}
		...
		env.tmc.setVRResults(
			primary.tablet,
			"select id, source, message, cell, tablet_types, workflow_type, workflow_sub_type, defer_secondary_keys from mysql.vreplication where db_name = 'target' and workflow = 'wrWorkflow'",
		...
		env.tmc.setVRResults(primary.tablet, "update mysql.vreplication set state = 'Stopped', message = 'for wrangler test' where db_name = 'target' and workflow = 'wrWorkflow'", &sqltypes.Result{RowsAffected: 1})
		env.tmc.setVRResults(primary.tablet, "update mysql.vreplication set state = 'Stopped' where db_name = 'target' and workflow = 'wrWorkflow'", &sqltypes.Result{RowsAffected: 1})
		env.tmc.setVRResults(primary.tablet, "delete from mysql.vreplication where message != '' and db_name = 'target' and workflow = 'wrWorkflow'", &sqltypes.Result{RowsAffected: 1})
		env.tmc.setVRResults(primary.tablet, "insert into mysql.vreplication(state, workflow, db_name) values ('Running', 'wk1', 'ks1'), ('Stopped', 'wk1', 'ks1')", &sqltypes.Result{RowsAffected: 2})
		...
		result := sqltypes.MakeTestResult(sqltypes.MakeTestFields(
			...
			fmt.Sprintf("1|%v|MySQL56/14b68925-696a-11ea-aee7-fec597a91f5e:1-3||0|Running|target|%d|0|%d|0|||||", bls, timeUpdated, timeUpdated),
		)
		env.tmc.setVRResults(primary.tablet, "select id, source, pos, stop_pos, max_replication_lag, state, db_name, time_updated, transaction_timestamp, time_heartbeat, time_throttled, component_throttled, message, tags, workflow_type, workflow_sub_type, defer_secondary_keys from mysql.vreplication where db_name = 'target' and workflow = 'wrWorkflow'", result)
		env.tmc.setVRResults(
			primary.tablet,
			"select source, pos from mysql.vreplication where db_name='target' and workflow='wrWorkflow'",
			sqltypes.MakeTestResult(sqltypes.MakeTestFields(
				"source|pos",
				"varchar|varchar"),
				posRows...,
		...
		env.tmc.setVRResults(primary.tablet, "select distinct workflow from mysql.vreplication where state != 'Stopped' and db_name = 'target'", result)
		...
		env.tmc.setVRResults(primary.tablet, "select table_name, lastpk from mysql.copy_state where vrepl_id = 1 and id in (select max(id) from mysql.copy_state where vrepl_id = 1 group by vrepl_id, table_name)", result)

		env.tmc.setVRResults(primary.tablet, "select id, source, pos, stop_pos, max_replication_lag, state, db_name, time_updated, transaction_timestamp, time_heartbeat, time_throttled, component_throttled, message, tags from mysql.vreplication where db_name = 'target' and workflow = 'bad'", &sqltypes.Result{})

		env.tmc.setVRResults(primary.tablet, "select id, source, pos, stop_pos, max_replication_lag, state, db_name, time_updated, transaction_timestamp, time_heartbeat, time_throttled, component_throttled, message, tags from mysql.vreplication where db_name = 'target' and workflow = 'badwf'", &sqltypes.Result{})
		...
		env.tmc.setVRResults(primary.tablet, "update mysql.vreplication set state='Running', message='', stop_pos='' where db_name='target' and workflow='wrWorkflow'", &sqltypes.Result{})
		...
		env.tmc.setVRResults(primary.tablet, "select distinct workflow from mysql.vreplication where db_name = 'target'", result)
		tabletID += 10
	}
	...
	env.tmc.setVRResults(primary.tablet, "select distinct workflow from mysql.vreplication where db_name = 'target2'", result)
	wranglerEnv = env
	return env
}
