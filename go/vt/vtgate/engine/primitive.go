...
type (
	...
	VCursor interface {
		...
		ResolveDefaultDestination(ctx context.Context, keyspace string, destination key.Destination) ([]*srvtopo.ResolvedShard, error)
		...
		ExecutorVSchemaAddKeyspaceIfNotExists(name string, KeyspaceSchema *vindexes.KeyspaceSchema)

		ExecutorVSchemaDeleteKeyspace(name string)
	}
