...
func TestModifyBranchSchemaDBNameToSourceDBName(t *testing.T) {
	tests := []struct {
		name        string
		input       *BranchSchema
		pattern     string
		expected    *BranchSchema
		expectError bool
	}{
		{
			...
			input: &BranchSchema{
				branchSchema: map[string]map[string]string{
					"target_example_db": {"table1": "CREATE TABLE table1 (...)"},
				},
			},
			pattern: fmt.Sprintf("target_%s_db", SourceDBNamePlaceHolder),
			expected: &BranchSchema{
				branchSchema: map[string]map[string]string{
					"example": {"table1": "CREATE TABLE table1 (...)"},
				},
			},
			expectError: false,
		},
		{
			name: "successful modification2",
			input: &BranchSchema{
				branchSchema: map[string]map[string]string{
					"target_example_db":  {"table1": "CREATE TABLE table1 (...)"},
					"target_example2_db": {"table2": "CREATE TABLE table2 (...)"},
				},
			},
			pattern: fmt.Sprintf("target_%s_db", SourceDBNamePlaceHolder),
			expected: &BranchSchema{
				branchSchema: map[string]map[string]string{
					"example":  {"table1": "CREATE TABLE table1 (...)"},
					"example2": {"table2": "CREATE TABLE table2 (...)"},
				},
			},
			expectError: false,
		},
		{
			name: "conflict due to existing source name",
			input: &BranchSchema{
				branchSchema: map[string]map[string]string{
					"target_example_db": {"table1": "CREATE TABLE table1 (...)"},
					"example":           {"table2": "CREATE TABLE table2 (...)"},
				},
			},
			pattern:     fmt.Sprintf("target_%s_db", SourceDBNamePlaceHolder),
			expectError: true,
		},
		{
			...
			input: &BranchSchema{
				branchSchema: map[string]map[string]string{
					"wrong_example_db": {"table1": "CREATE TABLE table1 (...)"},
				},
			},
			pattern:     fmt.Sprintf("target_%s_db", SourceDBNamePlaceHolder),
			expectError: true,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			rst, err := modifyBranchSchemaDBNameToSourceDBName(test.input, test.pattern)
			if test.expectError {
				if err == nil {
			...
			if !isNestedMapEqual(rst.branchSchema, test.expected.branchSchema) {
				t.Errorf("Expected branchSchema %v, but got %v", test.expected.branchSchema, test.input.branchSchema)
			}
		})
	}
}

func isNestedMapEqual(a, b map[string]map[string]string) bool {
	if len(a) != len(b) {
		return false
	}
	for k, v := range a {
		bv, exists := b[k]
		if !exists || !isMapEqual(v, bv) {
			return false
		}
	}
	return true
