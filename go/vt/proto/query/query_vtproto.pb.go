...
func (m *ExecuteOptions) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.TableReadAfterWriteGtidMap) > 0 {
		for k := range m.TableReadAfterWriteGtidMap {
			v := m.TableReadAfterWriteGtidMap[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
...
func (m *ExecuteOptions) SizeVT() (n int) {
	...
	if len(m.TableReadAfterWriteGtidMap) > 0 {
		for k, v := range m.TableReadAfterWriteGtidMap {
			_ = k
			_ = v
...
func (m *ExecuteOptions) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 22:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TableReadAfterWriteGtidMap", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.TableReadAfterWriteGtidMap == nil {
				m.TableReadAfterWriteGtidMap = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
			...
			m.TableReadAfterWriteGtidMap[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
