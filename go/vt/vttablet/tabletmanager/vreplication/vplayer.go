...
type vplayer struct {
	...
	timeOffsetNs int64
	// numAccumulatedHeartbeats keeps track of how many heartbeats have been received since we updated the time_updated column of mysql.vreplication
...
//
// settings: current settings read from mysql.vreplication.
...
func (vp *vplayer) fetchAndApply(ctx context.Context) (err error) {
	...
	go func() {
		streamErr <- vp.vr.sourceVStreamer.VStream(ctx, vp.vr.sourceTableSchema, mysql.EncodePosition(vp.startPos), nil, vp.replicatorPlan.VStreamFilter, func(events []*binlogdatapb.VEvent) error {
