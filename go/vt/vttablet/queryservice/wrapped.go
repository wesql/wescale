...
func (ws *wrappedService) SubmitDMLJob(ctx context.Context, cmd, sql, uuid, tableSchema string, timeGapInMs, subtaskRows int64, postponeLaunch, autoRetry bool) (qr *sqltypes.Result, err error) {
	err = ws.wrapper(ctx, nil, ws.impl, "SubmitDMLJob", false, nil, func(ctx context.Context, target *querypb.Target, conn QueryService) (bool, error) {
		...
		var innerErr error
		qr, innerErr = conn.SubmitDMLJob(ctx, cmd, sql, uuid, tableSchema, timeGapInMs, subtaskRows, postponeLaunch, autoRetry)
		return canRetry(ctx, innerErr), innerErr
	})
	return qr, err
}
