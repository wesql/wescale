...
type QueryService interface {
	...
	// ExecuteInternal for query execution by vtgate internal
	ExecuteInternal(ctx context.Context, target *querypb.Target, sql string, bindVariables map[string]*querypb.BindVariable, transactionID, reservedID int64, options *querypb.ExecuteOptions) (*sqltypes.Result, error)
	...
	SubmitDMLJob(ctx context.Context, command, sql, uuid, tableSchema, timePeriodStart, timePeriodEnd string, timeGapInMs, subtaskRows int64, postponeLaunch bool, failPolicy string) (*sqltypes.Result, error)
