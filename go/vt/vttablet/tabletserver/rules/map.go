...
func (qri *Map) FilterByPlan(query string, planType planbuilder.PlanType, tableNames ...string) (newqrs *Rules) {
	qri.mu.Lock()
	defer qri.mu.Unlock()
	newqrs = New()
	for _, rules := range qri.queryRulesMap {
		newqrs.Append(rules.FilterByPlan(query, planType, tableNames...))
	}
	return newqrs
}
