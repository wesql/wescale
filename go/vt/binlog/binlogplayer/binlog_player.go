...
var (
	...
	// VReplicationInit is for the CalculateDiff state.
	VReplicationInit = "CalculateDiff"
...
// replicating the provided keyrange and updating mysql.vreplication
...
// replicating the provided tables and updating mysql.vreplication
// with uid=startPosition.Uid.
// If !stopPosition.IsZero(), it will stop when reaching that position.
...
func (blp *BinlogPlayer) setVReplicationState(state, message string) error {
	...
	query := fmt.Sprintf("update mysql.vreplication set state='%v', message=%v where id=%v", state, encodeString(MessageTruncate(message)), blp.uid)
...
func ReadVRSettings(dbClient DBClient, uid uint32) (VRSettings, error) {
	query := fmt.Sprintf("select pos, stop_pos, max_tps, max_replication_lag, state, workflow_type, workflow, workflow_sub_type, defer_secondary_keys from mysql.vreplication where id=%v", uid)
...
func CreateVReplication(workflow string, source *binlogdatapb.BinlogSource, position string, maxTPS, maxReplicationLag, timeUpdated int64, dbName string,
	...
	return fmt.Sprintf("insert into mysql.vreplication "+
...
// GenerateUpdateRowsCopied returns a statement to update the rows_copied value in the mysql.vreplication table.
func GenerateUpdateRowsCopied(uid uint32, rowsCopied int64) string {
	return fmt.Sprintf("update mysql.vreplication set rows_copied=%v where id=%v", rowsCopied, uid)
}

// GenerateUpdateHeartbeat returns a statement to record the latest heartbeat in the mysql.vreplication table.
func GenerateUpdateHeartbeat(uid uint32, timeUpdated int64) (string, error) {
	if timeUpdated == 0 {
		return "", fmt.Errorf("timeUpdated cannot be zero")
	}
	return fmt.Sprintf("update mysql.vreplication set time_updated=%v, time_heartbeat=%v where id=%v", timeUpdated, timeUpdated, uid), nil
}

// GenerateUpdateTimeThrottled returns a statement to record the latest throttle time in the mysql.vreplication table.
func GenerateUpdateTimeThrottled(uid uint32, timeThrottledUnix int64, componentThrottled string) (string, error) {
	...
	return fmt.Sprintf("update mysql.vreplication set time_updated=%v, time_throttled=%v, component_throttled='%v' where id=%v", timeThrottledUnix, timeThrottledUnix, componentThrottled, uid), nil
}

// StartVReplication returns a statement to start the replication.
func StartVReplication(uid uint32) string {
	return fmt.Sprintf(
		"update mysql.vreplication set state='%v', stop_pos=NULL where id=%v",
		BlpRunning, uid)
}
...
func StartVReplicationUntil(uid uint32, pos string) string {
	return fmt.Sprintf(
		"update mysql.vreplication set state='%v', stop_pos=%v where id=%v",
...
func StopVReplication(uid uint32, message string) string {
	return fmt.Sprintf(
		"update mysql.vreplication set state='%v', message=%v where id=%v",
...
func DeleteVReplication(uid uint32) string {
	return fmt.Sprintf("delete from mysql.vreplication where id=%v", uid)
}
...
// given stream from the mysql.vreplication table.
func ReadVReplicationPos(index uint32) string {
	return fmt.Sprintf("select pos from mysql.vreplication where id=%v", index)
}
...
func ReadVReplicationStatus(index uint32) string {
	return fmt.Sprintf("select pos, state, message from mysql.vreplication where id=%v", index)
}
